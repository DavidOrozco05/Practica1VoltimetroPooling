...
# 11 "C:\\Program Files\\Microchip\\xc8\\v2.40\\pic\\include\\c99\\math.h" 2 3
...
# 1 "./nuestrodisplays.h" 1
# 40 "./nuestrodisplays.h"
...
void main(void)
{
 ...
 Tm_Inicie_periodico(&timer_ADC,200);
    ...
    while (1){
        if( TMR2_HasOverflowOccured()){
            Tm_Procese_tiempo (&timer_ADC);
            mostrar_Display(&miDisplay, num_Display);

  }


        if(Tm_Hubo_periodico(&timer_ADC)){
        ...
        if( ADC_IsConversionDone()&&convirtiendo){
            ...
            ADC2Display(&miDisplay, datoadc);

            llenar_registros_del_Serial(&miserial,miDisplay.Digito1,miDisplay.Digito2,miDisplay.Digito3);
        }
        if(EUSART_is_tx_ready() && tengodatos_PorEnviar(&miserial)){
            enviarsiguiente_Dato(&miserial);
        }
  }
}
